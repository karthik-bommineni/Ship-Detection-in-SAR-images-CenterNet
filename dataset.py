...
def random_horizontal_flip(img, bboxes, p=0.5):
    ...
    if random.random() < p:
        _, w_img, _ = img.shape
        img = img[:, ::-1, :]
        bboxes[:, 0] = w_img - bboxes[:, 0]   # 鍙樻崲鍦嗗績cx
        bboxes[:, 4] = 180 - bboxes[:, 4]
    bboxes = list(bboxes)
    return img, bboxes
...
def random_vertical_flip(img, bboxes, p=0.5):
    ...
    if random.random() < p:
        h_img, _, _ = img.shape
        img = img[::-1, :, :]
        bboxes[:, 1] = h_img - bboxes[:, 1]  # 鍙樻崲鍦嗗績cy
        bboxes[:, 4] = 180 - bboxes[:, 4]
    bboxes = list(bboxes)
    return img, bboxes
...
class ctDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 增加随机旋转代码
        gt_rboxes =  [ann['bbox'] for ann in anns]
        if self.split == 'train' and len(gt_rboxes) != 0:
            img, gt_rboxes = random_horizontal_flip(img, gt_rboxes)
            img, gt_rboxes = random_vertical_flip(img, gt_rboxes)
        # 可视化
        # draw(img,gt_rboxes)
        height, width = img.shape[0], img.shape[1]  
        c = np.array([img.shape[1] / 2., img.shape[0] / 2.], dtype=np.float32)  # 图像的中心点，用于计算放缩系数
        keep_res = False #
        if keep_res:
            input_h = (height | 31) + 1   
            input_w = (width | 31) + 1
        ...
        for k in range(num_objs):  # num_objs图中标记物数目  
            ...
            bbox, an = coco_box_to_bbox(gt_rboxes[k])
            cls_id = int(self.cat_ids[ann['category_id']]) 
            bbox[:2] = affine_transform(bbox[:2], trans_output)    # 图像是512*512， 而gt_label是128*128： [lx, ly]
            bbox[2:] = affine_transform(bbox[2:], trans_output)    # 图像是512*512， 而gt_label是128*128： [rx, ry]
            ...
            if h > 0 and w > 0:
                ...
                wh[k] = 1. * w, 1. * h
                ang[k] = 1. * an
                ind[k] = ct_int[1] * output_w + ct_int[0]  # ind = cy * 128 + cx
                reg[k] = ct - ct_int
                cxcy[k] = ct
                reg_mask[k] = 1                            # reg_mask只是统计第k个物体
        ret = {'input': inp, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh, 'ang':ang, 'cxcy': cxcy}
